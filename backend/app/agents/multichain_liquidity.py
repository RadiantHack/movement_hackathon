"""
Multichain Liquidity Agent - Web3 Liquidity Pool Information Agent

This module implements an AI-powered agent that helps users check liquidity pool
information across multiple blockchain networks (Ethereum, BNB, Polygon, etc.).
...
1. LangGraph Agent Layer:
   ...
   - Has access to tools: get_liquidity_pool(), get_pool_apy(), get_pool_tvl()
   - Uses a system prompt that guides the agent on how to handle liquidity queries
...
WORKFLOW:
---------
1. User sends a query (e.g., "get liquidity for USDC/ETH pool on ethereum")
2. RequestContext captures the user input
3. MultichainLiquidityAgentExecutor.execute() is called
4. MultichainLiquidityAgent.invoke() processes the query:
   - Validates OpenAI API key
   - Invokes LangGraph agent with user query
   - Agent uses tools to fetch liquidity data (currently hardcoded)
   - Extracts assistant response from agent result
5. Response is formatted as JSON and sent back via EventQueue

KEY COMPONENTS:
---------------
- MultichainLiquidityAgent: Core agent class that wraps LangGraph agent and ADK Runner
- MultichainLiquidityAgentExecutor: Implements A2A AgentExecutor interface
- Tools: get_liquidity_pool(), get_pool_apy(), get_pool_tvl() for liquidity queries
...
- ITINERARY_PORT: Optional - Server port (default: 9002)
- RENDER_EXTERNAL_URL: Optional - External URL for agent card

USAGE:
------
Standalone mode:
    python -m app.agents.multichain_liquidity.agent
    # Server starts on http://0.0.0.0:9002 (or ITINERARY_PORT)

Mounted mode:
    from app.agents.multichain_liquidity.agent import create_server
    app.mount("/liquidity", create_server(base_url="http://localhost:8000/liquidity"))

NOTES:
------
- Liquidity fetching tools use hardcoded mock data
...
def get_system_prompt() -> str:
    ...
    return """You are a helpful Web3 assistant specializing in liquidity pool information.

When users ask about liquidity:
1. Extract the token pair if provided (e.g., USDC/ETH, USDT/BNB)
2. Determine which network they're asking about (default to ethereum if not specified)
3. Identify what information they want (TVL, APY, pool details, etc.)
4. Use the appropriate tool to fetch liquidity data
5. Present results in a clear, user-friendly format

If the user doesn't provide a token pair, politely ask for it.
Always validate that addresses start with 0x and are 42 characters long if provided.
...
def create_agent_skill() -> AgentSkill:
    ...
    return AgentSkill(
        ...
        name="Multichain Liquidity Agent",
        description="Agent for checking liquidity pool information across multiple chains",
        tags=["liquidity", "defi", "pools", "ethereum", "bnb", "polygon", "web3"],
        examples=[
            "get liquidity for USDC/ETH",
            "get pool APY for USDT/BNB on bnb",
            "get TVL for DAI/ETH pool on ethereum",
            "check liquidity pool details",
            "what's the APY for USDC/USDT on polygon",
        ],
    )
...
def create_agent_card(port: int) -> AgentCard:
    ...
    return AgentCard(
        ...
        description=(
            ...
            "liquidity pool information across multiple chains"
        ),
        url=card_url,
        version="1.0.0",
...
@tool
def get_liquidity_pool(token_a: str, token_b: str, network: str = DEFAULT_NETWORK) -> str:
    """Get liquidity pool information for a token pair on a specific network.

    Args:
        token_a: First token symbol (e.g., USDC, ETH, USDT)
        token_b: Second token symbol (e.g., ETH, BNB, DAI)
        network: The blockchain network (ethereum, bnb, polygon, etc.)

    Returns:
        The liquidity pool information as a string
    """
    # Hardcoded mock data
    return (
        f"Liquidity Pool {token_a.upper()}/{token_b.upper()} on {network}:\n"
        f"- Total Value Locked (TVL): $2,450,000\n"
        f"- Token A Reserve: 1,250,000 {token_a.upper()}\n"
        f"- Token B Reserve: 850 {token_b.upper()}\n"
        f"- Pool Address: 0x1234567890123456789012345678901234567890"
    )


@tool
def get_pool_apy(token_a: str, token_b: str, network: str = DEFAULT_NETWORK) -> str:
    """Get the APY (Annual Percentage Yield) for a liquidity pool.
    ...
    Returns:
        The pool APY as a string
    """
    # Hardcoded mock data
    return (
        f"APY for {token_a.upper()}/{token_b.upper()} pool on {network}:\n"
        f"- Current APY: 12.5%\n"
        f"- 7-day Average: 11.8%\n"
        f"- 30-day Average: 10.2%"
    )


@tool
def get_pool_tvl(token_a: str, token_b: str, network: str = DEFAULT_NETWORK) -> str:
    """Get the TVL (Total Value Locked) for a liquidity pool.
    ...
    Returns:
        The pool TVL as a string
    """
    # Hardcoded mock data
    return (
        f"TVL for {token_a.upper()}/{token_b.upper()} pool on {network}:\n"
        f"- Current TVL: $2,450,000\n"
        f"- 24h Change: +5.2%\n"
        f"- 7d Change: +12.8%"
    )
...
def get_tools() -> List[Any]:
    ...
    return [get_liquidity_pool, get_pool_apy, get_pool_tvl]
...
class MultichainLiquidityAgent:
    def __init__(self):
        ...
        self._runner = Runner(
            app_name="multichainliquidityagent",
            agent=self._agent,
...
class MultichainLiquidityAgentExecutor(AgentExecutor):
    def __init__(self):
...
def create_multichain_liquidity_agent_app(card_url: str) -> A2AStarletteApplication:
    """Create and configure the A2A server application for the multichain liquidity agent.

    Args:
        card_url: The base URL where the agent card will be accessible

    Returns:
        A2AStarletteApplication instance configured for the multichain liquidity agent
    """
    ...
    request_handler = DefaultRequestHandler(
        agent_executor=MultichainLiquidityAgentExecutor(),
...
if __name__ == "__main__":
    port = get_port()
    card_url = get_card_url(port)
    app = create_multichain_liquidity_agent_app(card_url)
