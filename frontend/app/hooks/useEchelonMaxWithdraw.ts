/**
 * Hook to get the maximum withdrawable amount from Echelon on-chain contract
 * Uses the official SDK's getAccountWithdrawable method which queries the contract directly
 */
...
interface UseEchelonMaxWithdrawResult {
...
/**
 * Hook to fetch the maximum withdrawable amount for a specific asset from Echelon contract
 * @param marketAddress - The market address for the asset
 * @param decimals - The decimals of the asset (to convert from raw units)
 * @returns Max withdrawable amount, loading state, error, and refresh function
 */
export function useEchelonMaxWithdraw(
  marketAddress: string | null | undefined,
  decimals: number = 8
): UseEchelonMaxWithdrawResult {
  ...
  const fetchMaxWithdrawable = useCallback(async () => {
    ...
    try {
      ...
      const maxWithdrawableAmount = rawWithdrawable / Math.pow(10, decimals);

      setMaxWithdrawable(maxWithdrawableAmount);
    } catch (err) {
      console.error(
        "[useEchelonMaxWithdraw] Error fetching max withdrawable:",
        err
      );
      setError(
        err instanceof Error ? err.message : "Failed to fetch max withdrawable"
      );
      setMaxWithdrawable(null);
    } finally {
      setLoading(false);
